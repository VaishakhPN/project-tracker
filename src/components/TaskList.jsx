...
const TaskList = () => {
  ...
  const [catogs, setCatigs] = useState(null)
  ...
  const fetchTaskData = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/tickets/pageable?page=${pageNumber}&size=5&sort=ticketId,asc`);
      console.log('Task data from backend:', response.data);
      setTaskData(response.data.content);
      setData(response.data.content)

      if (response.data) {
        setSelectedTask(response.data.content[0]);
      }
    } catch (error) {
 ...
 let  category = null
  ...
  const fetchData = async (category) => {
    try {
      let response;
      if (category === "All") {
        response = await axios.get(`http://localhost:8080/tickets/pageable?page=0&size=5&sort=ticketId,asc`);
        setData(response.data.content);
          ...
          {selectedTask ? (
            <div className="bg-gray-100 p-4 rounded-lg">
              ...
              {isEditing ? (
                ...
                <div>
<h3>CATEGORY: {
  (category = catogs.find(category => category.categoryId === selectedTask.categoryId))
    ? category.name
    : 'Unknown'
}</h3>
